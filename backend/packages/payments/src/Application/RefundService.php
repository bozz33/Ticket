...
class RefundService
{
    ...
    public function quote(PlatformTransaction $transaction, ?string $reasonCode = null): array
    {
        ...
        if ($this->usesGlobalFinancePolicy($pricingSnapshot)) {
            return $this->quoteFromGlobalFinancePolicy($transaction, $pricingSnapshot, $reasonCode);
        }
    ...
    public function create(PlatformTransaction $transaction, array $payload = [], ?PlatformUser $actor = null): Refund
    {
        ...
        $quote = $this->quote($transaction, $reasonCode);
    ...
    private function usesGlobalFinancePolicy(array $pricingSnapshot): bool
    {
    ...
    {
        $subtotal = (int) ($pricingSnapshot['subtotal'] ?? max(0, (int) $transaction->gross_amount - (int) $transaction->customer_fee_amount));
        $commissionAmount = (int) ($pricingSnapshot['commission_amount'] ?? $pricingSnapshot['platform_fee_amount'] ?? $transaction->platform_fee_amount ?? 0);
        $cardFeeTotal = (int) ($pricingSnapshot['card_fee_total']
            ?? data_get($pricingSnapshot, 'breakdown.customer_fee.total')
            ?? $transaction->customer_fee_amount);
        $gatewayFeeAmount = (int) ($pricingSnapshot['gateway_fee_amount'] ?? $transaction->gateway_fee_amount ?? 0);
        $cardFeeRefunded = $this->financePolicyService->cardFeeRefundable($reasonCode) ? $cardFeeTotal : 0;
        $customerRefund = max(0, $subtotal + $cardFeeRefunded);
        $organizerReversal = max(0, $subtotal);
        $platformFeeReversed = $commissionAmount;
        $gatewayFeeRetained = $gatewayFeeAmount;

        return [
            'original_transaction_reference' => $transaction->transaction_reference,
            'subtotal_amount' => $subtotal,
            'customer_refund_amount' => $customerRefund,
            'organizer_reversal_amount' => $organizerReversal,
            'platform_absorption_amount' => max(0, $cardFeeRefunded + $gatewayFeeRetained),
            'platform_fee_retained' => 0,
            'gateway_fee_retained' => $gatewayFeeRetained,
            'tax_retained' => 0,
            'platform_fee_reversed' => $platformFeeReversed,
            'gateway_fee_reversed' => 0,
            'tax_reversed' => 0,
            'customer_fee_refunded' => $cardFeeRefunded,
            'currency_code' => strtoupper((string) ($transaction->currency_code ?: 'XOF')),
            'components' => [
                'platform' => [
