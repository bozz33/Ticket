...
class PublicContentService
{
    ...
    public function find(string $module, string $slug): ?array
    {
        ...
        $record = $modelClass::query()
            ...
            ->when($module === 'evenements', fn (Builder $query) => $query->withCount('likes'))
    ...
    public function availableFilters(?string $module = null): array
    {
        $categories = Category::query()
            ->where('is_active', true)
            ->when(
                $this->moduleScope($module) !== null,
                fn (Builder $query) => $query->whereIn('module_scope', [CategoryScope::Global->value, $this->moduleScope($module)]),
            )
            ->orderBy('sort_order')
            ->orderBy('name')
            ->pluck('name')
        ...
        foreach ($modules as $moduleName) {
            $modelClass = self::MODULE_MAP[$moduleName];
    ...
    public function summarySnapshotAcrossTenants(): array
    {
        ...
        $moduleCards = collect(array_keys(self::MODULE_MAP))
            ->map(function (string $module) use ($items): array {
                $presentation = $this->modulePresentation($module);
    ...
    private function searchSuggestions(string $query, array $modules, int $limit): Collection
    {
        ...
        foreach ($modules as $module) {
            $modelClass = self::MODULE_MAP[$module];
    ...
    private function moduleScope(?string $module): ?string
    {
        return match ($module) {
    ...
    private function modulePresentation(string $module): array
    {
