...
class PaymentWebhookService
{
    ...
    protected function syncTransaction(PaymentGateway $gateway, array $payload): ?PlatformTransaction
    {
        ...
        $pricingSnapshot = (array) ($existingTransaction?->pricing_snapshot ?? data_get($existingMeta, 'pricing', []));
        ...
        $totalFees = $platformFees + $gatewayFees + $taxAmount;
    ...
    {
        if (! $transaction || ! $this->isSuccessfulStatus((string) $transaction->status)) {
            return;
        }
