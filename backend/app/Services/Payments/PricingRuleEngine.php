...
class PricingRuleEngine
{
    public function quote(
        ...
        $platformRule = $this->resolvePlatformFeeRule($tenant, $module, $countryCode, $currencyCode);
        $platformFee = $this->evaluateFeeRule(
            baseAmount: $subtotal,
            feeMode: $platformRule['fee_mode'],
            percentageRate: $platformRule['percentage_rate'],
            fixedAmount: $platformRule['fixed_amount'],
            capAmount: $platformRule['cap_amount'],
            vatRate: $platformRule['vat_rate'],
        );

        $baseBeforeGateway = $subtotal + ($platformRule['charge_bearer'] === FeeChargeBearer::Buyer->value ? $platformFee['total'] : 0);
        $gatewayRule = $gateway ? $this->resolveGatewayFeeRule($gateway, $countryCode, $currencyCode, $paymentChannel) : null;
        $gatewayFee = $this->evaluateGatewayFeeRule($baseBeforeGateway, $subtotal, $gatewayRule);

        $buyerFeeTotal = 0;
        $organizerFeeTotal = 0;
        $platformAbsorbedTotal = 0;

        foreach ([
            ['rule' => $platformRule, 'fee' => $platformFee],
            ['rule' => $gatewayRule, 'fee' => $gatewayFee],
        ] as $entry) {
            $rule = $entry['rule'];
            $fee = $entry['fee'];

            if (! is_array($rule) || $fee['total'] <= 0) {
                continue;
            }

            match ($rule['charge_bearer']) {
                FeeChargeBearer::Buyer->value => $buyerFeeTotal += $fee['total'],
                FeeChargeBearer::Organizer->value => $organizerFeeTotal += $fee['total'],
                FeeChargeBearer::Platform->value => $platformAbsorbedTotal += $fee['total'],
                default => null,
            };
        }

        $customerTotal = $subtotal + $buyerFeeTotal;
        $organizerNet = max(0, $subtotal - $organizerFeeTotal);
        $totalTax = $platformFee['tax_amount'] + $gatewayFee['tax_amount'];
        $totalFeeAmount = $platformFee['base_amount'] + $gatewayFee['base_amount'] + $totalTax;

        return [
            'subtotal' => $subtotal,
            'service_fee' => $buyerFeeTotal,
            'customer_fee_total' => $buyerFeeTotal,
            'organizer_fee_total' => $organizerFeeTotal,
            'absorbed_fee_total' => $platformAbsorbedTotal,
            'gateway_fee_amount' => $gatewayFee['base_amount'],
            'platform_fee_amount' => $platformFee['base_amount'],
            'tax_amount' => $totalTax,
            ...
            'module' => $module->value,
            'policy' => $this->legacyPolicySummary($platformRule),
            'breakdown' => [
                'platform_fee' => [
                    ...$platformFee,
                    'charge_bearer' => $platformRule['charge_bearer'] ?? null,
                    'rule' => $this->ruleSummary($platformRule),
                ],
                'gateway_fee' => [
                    ...$gatewayFee,
                    'charge_bearer' => $gatewayRule['charge_bearer'] ?? null,
                    'rule' => $this->ruleSummary($gatewayRule),
                ],
                'totals' => [
                    'buyer_fee_total' => $buyerFeeTotal,
                    'organizer_fee_total' => $organizerFeeTotal,
                    'platform_absorbed_total' => $platformAbsorbedTotal,
                    'tax_total' => $totalTax,
                    'fee_total' => $totalFeeAmount,
                ],
            ],
        ];
    }
    ...
    private function buildFreeQuote(string $currencyCode, int $quantity, CommercialModule $module): array
    {
        return [
            'subtotal' => 0,
            'service_fee' => 0,
            'customer_fee_total' => 0,
            'organizer_fee_total' => 0,
            'absorbed_fee_total' => 0,
            ...
            'policy' => null,
            'breakdown' => [
                'platform_fee' => ['base_amount' => 0, 'tax_amount' => 0, 'total' => 0, 'charge_bearer' => null, 'rule' => null],
                'gateway_fee' => ['base_amount' => 0, 'tax_amount' => 0, 'total' => 0, 'charge_bearer' => null, 'rule' => null],
                'totals' => [
                    'buyer_fee_total' => 0,
                    'organizer_fee_total' => 0,
                    'platform_absorbed_total' => 0,
                    'tax_total' => 0,
                    'fee_total' => 0,
                ],
            ],
    ...
    private function pickBestRule(Collection $rules, array $context): ?array
    {
        ...
        return $rules
            ->filter(function ($rule) use ($now): bool {
    ...
    private function moduleFromOffer(Offer $offer): CommercialModule
    {
        return match ($offer->offerable_type) {
            \App\Models\CallForProject::class => CommercialModule::CallsForProjects,
            \App\Models\Training::class => CommercialModule::Training,
            \App\Models\Stand::class => CommercialModule::Stands,
            \App\Models\CrowdfundingCampaign::class => CommercialModule::Crowdfunding,
