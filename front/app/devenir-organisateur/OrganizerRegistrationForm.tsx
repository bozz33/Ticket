...
export function OrganizerRegistrationForm({ brandName }: { brandName: string }) {
  ...
  const [success, setSuccess] = useState<{ slug: string; accessUrl: string; loginUrl: string } | null>(null);
    ...
    return (
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "12px", padding: "32px", textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-soft)", marginBottom: "24px" }}>
          L&apos;espace <strong>{success.slug}</strong> a été créé sur {brandName}. Vous pouvez accéder directement à votre backoffice.
        </p>
        <a
          href={success.accessUrl}
          className="button"
