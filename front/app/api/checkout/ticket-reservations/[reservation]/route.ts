...
export async function DELETE(
  request: NextRequest,
  context: { params: Promise<{ reservation: string }> },
) {
  ...
  const rateLimitError = applyMutationRateLimit(request, "checkout-ticket-release", 20);
  ...
  const { reservation } = await context.params;

  try {
    const response = await fetch(`${apiBaseUrl}/api/v1/public/tenants/${encodeURIComponent(tenantSlug)}/ticket-reservations/${encodeURIComponent(reservation)}`, {
      method: "DELETE",
      headers: {
